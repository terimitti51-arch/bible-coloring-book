...
for i, prompt in enumerate(prompts):
    ...
    if response.status_code == 200:
        ...
        # Step 3: Art Effects Remove (Clean గా చేయడం)
        img = Image.open(img_path).convert("L")  # Grayscale
        img = img.filter(ImageFilter.SHARPEN)
        img = img.point(lambda x: 0 if x < 128 else 255)  # Black & White clean
        img.save(img_path)
    ...
    time.sleep(2)  # API limit కోసం wait

# Step 4: PDF తయారు చేయి
print("Creating PDF...")
with open("output/coloring_book.pdf", "wb") as f:
    f.write(img2pdf.convert(image_paths))

print("✅ PDF Ready! output/coloring_book.pdf")
